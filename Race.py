...
def shutdown_pybox():
    pybox_manager.shutdown(now=True)

# Ensure PyBox shuts down on exit
atexit.register(shutdown_pybox)
...
global time, delta, player_car_rect, collision_occurred, current_environment_index
time = True
delta = 0
SCREENHEIGHT = 240
SCREENWIDTH = 320
...
def game_loop ():
    global highscore, delta, time, total_time,  collision_occurred, car_sprite, current_song_index, tree_sprite, current_environment_index, music_playing, cars, trees, enemy_position_offset, start
    screen = pygame.display.set_mode (screen_size, pygame.SCALED)
    pygame.display.set_caption ("Don't Crash!!!")
